  ...
  const generateColumns = ({ onToggleTime, onTemplateClick, onDeleteData,handleRestriction }) => [
    {
      ...
      header: 'Name',
      Cell: ({ row }) =>   row.original.last_name ? row.original.first_name +' ' + row.original.last_name:row.original.first_name // Event/Trans/Acc/Other title
    },
    {
      accessorKey: 'user_name',
      header: 'User Name',
      Cell: ({ row }) => row.original.user_name || '-' // Event/Trans/Acc/Other title
    },
    {
      accessorKey: 'mobile',
      header: 'Mobile',
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <>
         {/* {row.original.is_restricted ? 'Yes' : 'No'} */}
        <Select
                labelId='is_restricted_label'
                label='Is Restricted?'
                name='is_restricted'
                value={row.original.is_restricted}
                onChange={e => handleRestriction(row.original.id, 'is_restricted', e.target.value)}
                disabled={row.original.role === 'main' && userData.role === 'super-admin'}
              >
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <>
          ...
          {row.original.role === 'main' && userData.role === 'super-admin' ? null : <Edit onClick={() => onTemplateClick(row.original)}/>}
