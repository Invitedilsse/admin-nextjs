...
const SideBarKeyType = props => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async values => {
      try {
        ...
        const result =
          id === ''
            ? await apiPost(`${baseURL}key-type/add`, params)
            : await apiPut(`${baseURL}key-type/update/${RowData.id}`, params)
        formik.resetForm()
  ...
  const getKeyTypes = async () => {
    try {
      ...
      console.log('Error fetching key type:', e)
    } finally {
      setIsLoading(false)
    }
  }
    ...
    >
      <Box className={classes.root}>
        <Box sx={{ p: theme => theme.spacing(0, 4, 4) }}>
          <Header>
            <Typography variant='h5'>{id !== '' ? 'Edit Key Type' : 'Create a Key Type'}</Typography>
            <IconButton
              size='small'
              onClick={handleClose}
              sx={{
