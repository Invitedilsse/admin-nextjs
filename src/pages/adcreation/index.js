...
import CreateAdvertisement from './components/createAdtemplate'
...
function AdTemplateList({ RowData }) {
  ...
  const generateColumns = ({ onToggleTime, onTemplateClick, onDeleteData }) => [
    ...
    },
    {
      ...
      Cell: ({ row }) =>`${row.original.header}`|| '-' // Event/Trans/Acc/Other title
    },
     {
      accessorKey: 'body',
      header: 'Body',
      ...
      accessorKey: 'footer',
      header: 'Footer',
      Cell: ({ row }) => row.original.footer || '-' // Event/Trans/Acc/Other title
    },
     {
      accessorKey: 'is_active',
      header: 'Status',
      Cell: ({ row }) => row.original.is_active ? 'Active' : 'Inactive' // Event/Trans/Acc/Other title
    }, {
      accessorKey: 'files',
      header: 'No of Files',
      Cell: ({ row }) => row.original.files.length || 0 // Event/Trans/Acc/Other title
    },
    {
      accessorKey: 'Action',
      header: 'action',
