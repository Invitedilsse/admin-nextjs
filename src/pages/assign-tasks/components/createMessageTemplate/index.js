...
function WaCallertriggerTemplateList({ RowData }) {
  ...
  const generateColumns = ({ onToggleTime, onTemplateClick, onDeleteData }) => [
    {
      accessorKey: 'tempalte_name',
      header: 'Template Name',
      Cell: ({ row }) => row.original.tempalte_name || '-' // Event/Trans/Acc/Other title
    },
   {
    accessorKey: 'fields',
    header: 'Values',
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await apiGet(`${getTemplateInputList}/${RowData?.id}`)
      console.log('Push Notification Templates:', response.data)
  ...
  return (
    <>
      ...
      {/* Template Drawer */}
      <CreateCustomMessageDrawerAdmin
...
export default WaCallertriggerTemplateList
