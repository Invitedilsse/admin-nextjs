...
function WatriggerTemplateList({ page }) {
  ...
  const generateColumns = ({ onToggleTime, onTemplateClick, onDeleteData }) => [
    {
      ...
      header: 'Family Name',
      Cell: ({ row }) => row.original.family_name || '-' // Event/Trans/Acc/Other title
    },
    {
      accessorKey: 'occasion_name',
      header: 'Occasion Name',
      Cell: ({ row }) => row.original.occasion_name || '-' // Event/Trans/Acc/Other title
    },
    {
      accessorKey: 'name',
      header: 'Name',
  ...
  const handleDeleteData = async id => {
    ...
    try {
      console.log('body')
      await apiDelete(deleteTemplateListWA + `?functionId=${functionId}&${delId}`)
      fetchData()
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await apiGet(`${getTempalteListWA}?functionId=${functionId}`)
      console.log('Push Notification Templates:', response.data)
...
export default WatriggerTemplateList
