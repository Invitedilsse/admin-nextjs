  ...
  const formik = useFormik({
    ...
    onSubmit: async values => {
      try {
        ...
        const result =
          id === ''
            ? await apiPost(`${baseURL}special-invitee/add`, params)
            : await apiPut(`${baseURL}special-invitee/update/${RowData.id}`, params)
        toggle()
        setTimeout(() => {
          formik.resetForm()
          setIsLoading(false)
  ...
  const deleteImage = async () => {
    ...
    try {
      ...
      if (result?.status == 200) {
        ...
        const response = await apiPut(`${baseURL}special-invitee/update/${RowData.id}`, params)
        if (response?.status == 200) {
          ...
          toast.success('Special invitee logo deleted successfully')
        }
      }
    } catch (e) {
      toast.error(e)
    ...
    >
      ...
      <Box className={classes.root}>
        <Box sx={{ p: theme => theme.spacing(0, 4, 4) }}>
          <Header>
            <Typography variant='h5'>{id !== '' ? 'Edit Special Invitee' : 'Create Special Invitee'}</Typography>
            <IconButton
              size='small'
              onClick={handleClose}
              sx={{
