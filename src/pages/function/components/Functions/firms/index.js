...
import AddFileDrawer from './components/ManageFirmsDrawer'
import { baseURL } from 'src/services/pathConst'
import { getListOfFirms } from 'src/store/adminMod'
...
const Firms = ({ isOpen }) => {
  ...
  const { isFirmsFetching, allFirms, firmsCount, functionId, functionData } = useSelector(state => state.adminMod)
    ...
    try {
      ...
      if (result?.data?.data) {
        if (type === 'firms') {
          ...
          setFiles(hostImages)
        } else if (type === 'firms_video') {
          const hostVideo = result?.data?.data
            ?.filter(item => item.type === 'firms_video')
            .map(item => ({
              id: item.id,
              other_id: item.other_id,
              type: item.type,
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...
    onDrop: acceptedFiles => {
      handleOtherMedia('firms', acceptedFiles[0])
    }
  })
  ...
  const { getRootProps: getVideoRootProps, getInputProps: getVideoInputProps } = useDropzone({
    ...
    onDrop: acceptedFiles => {
      ...
      if (acceptedFiles.length > 0) {
        ...
        handleOtherMedia('firms_video', acceptedFiles[0])
      }
    }
  })
  const deleteEvent = async () => {
    try {
      ...
      let url = `${baseURL}firm/delete/${delId}`
                        ...
                        secondaryAction={
                          <>
                            ...
                            <IconButton
                              ...
                              onClick={() => {
                                console.log('params--->frims', inviteVideo)

                                inviteVideo?.[0]?.key
                                  ? handleDeleteOtherMedia(inviteVideo[0].key, 'firms_video', inviteVideo[0].id)
                                  : setInviteVideo([])
