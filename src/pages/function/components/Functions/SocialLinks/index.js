...
const SocialLink = ({ isOpen }) => {
  ...
  const [rowCountState, setRowCountState] = useState(linkData.length || 0)

  const listSocialLink = async () => {
    try {
      setDataLoading(true)

      let url = `${baseURL}function/listSocialLinksWeb/${functionId}`

      const result = await apiGet(url)
      console.log('listSocialLink----->', result?.data)
  ...
  const deleteEvent = async () => {
    try {
      ...
      let url = `${baseURL}function/deleteSocialLink?function_id=${functionId}&id=${delId}`
  ...
  const columns = [
...

    {
      ...
      Cell: ({ renderedCellValue, row }) => (
        <>
          <Avatar
            src={`${(row?.original?.logo && row?.original?.logo?.url) || 'NA'}`}
  ...
  const table = useMaterialReactTable({
    ...
    muiLinearProgressProps: ({ isTopToolbar }) => ({
      ...
      value: dataLoading //show precise real progress value if you so desire
    }),
