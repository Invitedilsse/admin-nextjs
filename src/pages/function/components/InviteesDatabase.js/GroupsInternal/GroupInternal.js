...
import AddFileDrawer from './components/ManageGroupsInternalDrawer'
import { baseURL } from 'src/services/pathConst'
import { getListOfFunctionContacts, getListOfFunctionGroupsInternal } from 'src/store/adminMod'
...
const Group = () => {
  ...
  const { isGroupsInterFunctionFetching, groupsInterFunctionAll, groupsInterFunctionCount, functionId } = useSelector(state => state.adminMod)
  ...
  const [rowCountState, setRowCountState] = useState(groupsInterFunctionCount || 0)
  ...
  useEffect(() => {
    setRowCountState(prevRowCountState => (groupsInterFunctionCount !== undefined ? groupsInterFunctionCount : prevRowCountState))
  }, [groupsInterFunctionCount, setRowCountState])
  ...
  const deleteEvent = async () => {
    try {
      ...
      let url = `${baseURL}invitee-internal-group/delete/${delId}`
  ...
  const columns = [
    ...
    },
    {
      ...
      Cell: ({ row }) => `${row?.original?.contact_details?.length || 0}`
    },
    {
      accessorKey: 'action',
      enableSorting: false,
  ...
  const table = useMaterialReactTable({
    ...
    muiLinearProgressProps: ({ isTopToolbar }) => ({
      ...
      value: isGroupsInterFunctionFetching //show precise real progress value if you so desire
    }),
    ...
    state: {
      pagination,
      showProgressBars: isGroupsInterFunctionFetching,
      isLoading: isGroupsInterFunctionFetching,
      columnFilters: [],
      globalFilter: ''
    }
  });
        ...
        <Grid2 container spacing={2} px={4} display={'flex'} justifyContent={'end'}>
          <Grid2 size={{ xs: 12, lg: 6, md: 6, sm: 12 }} >
            <TextField
              ...
              fullWidth
              sx={{ mr: 4 }}
              placeholder='Search Internal Group'
