...
function CustomPushNotificationMappingSelected({}) {
  ...
  const handleUpdateMappingColumn = async ({ msg_id, checked }) => {
    try {
      ...
      await apiPatch(`${updateCustomMappedUserBulkSelected}/${msg_id}`, {
        contact_ids: contactIds,
        checked,
        functionId,
        oid: selectedFil,
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await apiGet(
        `${postCustomSelectedMappedUser}/${functionId}?search_string=${searchText}&oid=${selectedFil}&page=${params.page}&limit=${params.size}`
      )
      console.log('Custom Push Notification Mapping:', response.data)
...
export default CustomPushNotificationMappingSelected
