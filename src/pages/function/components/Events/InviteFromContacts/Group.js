...
const Group = () => {
  ...
  const { isGroupdFetching, allGroups, groupsCount,eventId } = useSelector(state => state.adminMod)
  ...
  const getAllGiftTypes =async () => {
    ...
    // dispatch(getListOfGroups(queryParams))
     try{
          setIsLoading(true)
          const response= await apiGet(`${baseURL}event-user-group/list?${queryParams}`)
  ...
  const deleteEvent = async () => {
    try {
      ...
      let url = `${baseURL}event-user-group/update`
console.log(params)
// return
