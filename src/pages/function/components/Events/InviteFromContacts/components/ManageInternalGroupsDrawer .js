...
const SideBarGiftType = props => {
  ...
  const getAllGiftTypes = () => {
    ...
    const queryParams = `function_id=${functionId}&limit=${2000}&offset=${0}&search_string=${searchText}&sortDir=${sortModel.length > 0 ? sortModel[0]?.sort : 'desc'}&sortBy=${sortModel.length > 0 ? sortModel[0]?.field : 'title'}`
  ...
  const addGroups = async () => {
    ...
    try {
      ...
      console.log(params)
      setIsLoading(true)
      const result = await apiPost(`${baseURL}event-internal-user-group/add`, params)
      console.log(result)
  ...
  return (
    ...
    >
      <Box className={classes.root}>
        <Box sx={{ px: 5, py: 2 }}>
          <Header>
            <Typography variant='h5'>{'Add from Internal Group'}</Typography>
            <IconButton
              size='small'
              onClick={handleClose}
              sx={{
