...
import { getListOfContacts, getListOfContactsNP, getListOfGroups } from 'src/store/adminMod'
...
const SideBarGiftType = props => {
  ...
  const { isGroupdFetching, allGroups, groupsCount, isContactsFetching, allContacts, contactsCount, eventId } =
  ...
  const getAllGiftTypes = () => {
    ...
    const queryParams = `limit=${2000}&offset=${0}&search_string=${searchText}&sortDir=${sortModel.length > 0 ? sortModel[0]?.sort : 'desc'}&sortBy=${sortModel.length > 0 ? sortModel[0]?.field : 'title'}`
  ...
  const columns = [
    ...
    },
    {
      ...
      Cell: ({ renderedCellValue, row }) => `${row?.original?.contact_id?.length || 0}`
  ...
  const addGroups = async () => {
    ...
    try {
      const data =
        contactData?.detail?.length > 0 && contactData?.detail[0]?.user_group_id?.length > 0
          ? contactData?.detail[0]?.user_group_id
          : []
      ...
      const params = {
        ...
        event_id: [eventId],
        user_group_id: [...data, ...selectedContacts]
      }
      console.log(params)
      setIsLoading(true)
      const result = await apiPost(`${baseURL}event-user-group/add`, params)
      console.log(result)
  ...
  return (
    ...
    >
      <Box className={classes.root}>
        <Box sx={{ px: 5, py: 2 }}>
          <Header>
            <Typography variant='h5'>{'Add from Group'}</Typography>
            <IconButton
              size='small'
              onClick={handleClose}
              sx={{
