...
import { handleEventReminderDT } from 'src/store/adminMod'
...
const SideBarSpecialInvitee = props => {
  // ** Props
  const { open, toggle, id, RowData, funId } = props
  ...
  useEffect(() => {
    if (formik.values.scheduledDateTime && formik.values.scheduledDateTime !== '') {
      dispatch(
        handleEventReminderDT({ data: moment(formik.values.scheduledDateTime).toISOString(), booleanval: false })
      )
    }
        ...
        <Box sx={{ p: theme => theme.spacing(0, 8, 8), width: '100% !important' }}>
          ...
          <Fragment>
            ...
            <Accordion>
              ...
              <AccordionDetails>
                <CustomMediaEvents getAll={() => {}} id={funId} />
              </AccordionDetails>
            </Accordion>
          </Fragment>
          <Box
