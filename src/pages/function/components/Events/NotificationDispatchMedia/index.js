...
const NotificationsDispatchMedia = ({ isOpen,id }) => {
  ...
  const table = useMaterialReactTable({
    ...
    muiLinearProgressProps: ({ isTopToolbar }) => ({
      ...
      value: isNotificationsDispatchMediaFetching //show precise real progress value if you so desire
    }),
  ...
  return (
    <Grid2 container spacing={6}>
          ...
          <Grid2 container spacing={2} px={4} display={'flex'} justifyContent={'end'}>
            ...
            <Grid2 size={{ xs: 12, lg: 2, md: 2, sm: 12 }} >
              <Button
                ...
                color='primary'
                sx={{ '& svg': { mr: 2 } }}
                disabled={allNotificationsDispatchMedia.length===1}
              >
                Add Reminder
              </Button>
            </Grid2>
      ...
      <Box p={4}>
        <AddFileDrawer open={addUserOpen} toggle={toggleAddUserDrawer} id={eid} RowData={entireRow} funId={id} />
      </Box>
