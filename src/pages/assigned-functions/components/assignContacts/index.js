...
function ListContactsCallers({RowData}) {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await apiGet(
        `${getContactListCallers}?functionId=${RowData.id}&page=${pagination.pageIndex + 1}&limit=${pagination.pageSize}&search=${searchText}&filterbyoid=${filval}`
      )
