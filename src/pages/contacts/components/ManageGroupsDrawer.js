...
const SideBarGiftType = props => {
  ...
  const { isContactsFetchingNP, allContactsNP } = useSelector(state => state.adminMod)
      ...
      try {
        ...
        const result =
          groupId === ''
            ...
            : await apiPut(`${baseURL}user-group/update/${groupId}`, {
              title: values.name,
              description: values.description,
              contact_id: values.contact_id
            })
  ...
  const handleClose = () => {
    if (groupId !== '' || formik.values.name == '' || formik.values.description == '') {
      setGroupId('')
      formik.resetForm()
    }
    toggle()
  ...
  const getAllContactsNP = () => {
    const queryParams = `search_string=${searchText}&sortDir=${'desc'}&sortBy=${'created_at'}`
  ...
  const table = useMaterialReactTable({
    ...
    onRowSelectionChange: setRowSelectionUpdate,
    ...
    >
      <Box className={classes.root}>
        <Box sx={{ p: theme => theme.spacing(0, 4, 4) }}>
          <Header>
            <Typography variant='h5'>{groupId !== '' ? 'Edit Group' : 'Create a Group'}</Typography>
            <IconButton
              size='small'
              onClick={handleClose}
              sx={{
